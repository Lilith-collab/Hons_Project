...
# Create a model of gg weight vs gg length for table


gg_al <- gg_avg_length %>%   #extracting avg length
arrange(Tank_No) %>% 
summarise(Treatment = Treatment, Avg_Length = Avg_Length, Row_ID = row_number(), .groups = "drop")

gg_aw <- gg_avg_weight %>%    #extracting avg weight
arrange(Tank_No) %>% 
summarise(Avg_Weight = Avg_Weight,Row_ID = row_number(), .groups = "drop")


gg_aov_merged <- merge(gg_al,gg_aw, by = "Row_ID" ) 
  
  mAOV <- aov(Avg_Weight ~ Avg_Length, data = gg_aov_merged)

  gg_avg_length_anova <- as.data.frame(summary(mAOV)[[1]]) %>%
...
anova_df %>%
  ...
  tab_spanner(
    label = "ANOVA Results by Treatment",
    columns = everything()
  )
...
# Extract dfs for each model
get_dfs <- function(model) {
  a <- summary(model)[[1]]
  # numerator df is usually first row (treatment), denominator df is residuals row
  df1 <- a["AW", "Df"] # adjust "AW" if your predictor has different name
  df2 <- a["Residuals", "Df"]
  c(df1 = df1, df2 = df2)
}

# Calculate critical F at 0.01 for each model
critical_Fs <- tibble(
  Treatment = c("Control", "5", "10"),
  df1 = c(get_dfs(mcon)["df1"], get_dfs(m5)["df1"], get_dfs(m10)["df1"]),
  df2 = c(get_dfs(mcon)["df2"], get_dfs(m5)["df2"], get_dfs(m10)["df2"])
...
# Join critical F values to anova_df by Treatment
anova_df <- anova_df %>%
  left_join(critical_Fs, by = "Treatment")

# Format with gt
anova_df %>%
  filter(Source != "Residuals") %>%  # usually exclude residual row
...
"If you want by tank number maybe if you want tank number"
 geom_text(
    label=TanK, 
