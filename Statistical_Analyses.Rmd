...
gg_avg_rgr_length <- rgr_weekly_sd %>%
  ...
  SD_RGR_Length = mean(SD_RGR, na.rm = TRUE), N = n(), # not the sd bro   # need SD still
...
#Bar Graph for rgr percentage of length of gg
...
ggplot(gg_avg_rgr_length, aes(x = factor(Week), y = RGR_Per, fill = Treatment)) +
  ...
  labs(
    title = "Relative Growth Rate Length (%) by Week and Treatment",
    x = "Week",
    y = "RGR (%)",
    fill = "Treatment"
  ) +
...
ggplot(gg_avg_rgr_weight, aes(x = factor(Week), y = RGR_Per, fill = Treatment)) +
  ...
  scale_fill_manual(values = c("black", "grey50","grey")) + 
  labs(
    title = "Relative Growth Rate Weight (%) by Week and Treatment",
    x = "Week",
    y = "RGR (%)",
    fill = "Treatment"
  ) +
...
# Bar graph of RGR Weight of GL
 
ggplot(gl_avg_rgr_weight, aes(x = factor(Week), y = RGR_Per, fill = Treatment)) +
  geom_col(position = position_dodge(width = 0.8), color = "black") +
  scale_fill_manual(values = c("black", "grey50","grey")) + 
  labs(
    title = "Relative Growth Rate Weight (%) by Week and Treatment of GL",
    x = "Week",
    y = "RGR (%)",
    fill = "Treatment"
  ) +
...
anova_df %>%
  ...
  tab_header(
    ...
    subtitle = md("*Gracilaria gracilis* — grouped by cutting length")
  ) %>%
  tab_options(
    table.font.size = px(14),
    data_row.padding = px(5),
    table_body.hlines.style = "none" # no horizontal lines between Regression/Residual/Total
  ) %>%
  tab_style( # Add a separating line only after Total
...
anova_df %>%
  ...
  tab_header(
    title = md("Analysis of variance between thallus length and net wet weight"),
    subtitle = md("*Gracilariopsis longissima* — grouped by cutting length")
  ) %>%
  tab_options(
    table.font.size = px(14),
    data_row.padding = px(5),
...
anova_df %>%
  ...
  tab_options(
    table.font.size = px(14),
    data_row.padding = px(10),  # more height so centering is obvious
    table_body.hlines.style = "none"
  ) %>%
  tab_style(
    style = cell_text(v_align = "middle"),  # vertical alignment for the column
    locations = cells_body(columns = "Cutting length")
  ) %>%
  tab_style( # line only after "Total" row
    style = cell_borders(sides = "bottom", color = "black", weight = px(1)),
    locations = cells_body(rows = Source == "Total")
  )
...
anova_df <- bind_rows(
  ...
  left_join(critical_Fs, by = "Treatment") %>%
  rename(`Cutting length` = Treatment)  # rename for table display

anova_df %>%
  gt(rowname_col = "Source") %>%   # keeps Regression, Residual, Total as row labels
  fmt_number(
    columns = where(is.numeric),
    decimals = 2
  ) %>%
...
tidy_anova <- function(model, treatment_name) {
  ...
  # Extract Residual row
  res <- a[rownames(a) == "Residuals", ]
  res_df <- data.frame(
    Treatment = treatment_name,
    Source = "Residual",
    `Sum of squares` = res[["Sum Sq"]],
    df = res[["Df"]],
    `Mean squares` = res[["Mean Sq"]],
    `F value` = NA,  # no F value for residuals
    check.names = FALSE
  )
...
# Function to extract only regression row from aov()
tidy_anova <- function(model, treatment_name) {
  a <- summary(model)[[1]]
  a <- a[rownames(a) != "Residuals", ] # remove residuals
  data.frame(
    Treatment = treatment_name,
...
# Combine ANOVAs — only regression rows
anova_df <- bind_rows(
  tidy_anova(mcon, "Control"),
  tidy_anova(m5, "5"),
  tidy_anova(m10, "10")
...
# Final table — one row per treatment
anova_df %>%
...
anova_df %>%
  ...
  tab_header(
    title = md("*Analysis of variance between thallus length and net wet weight*"),
    subtitle = md("Gracilaria chorda — grouped by cutting length")
  ) %>%
  tab_options(
    table.font.size = px(14),
    data_row.padding = px(5)
